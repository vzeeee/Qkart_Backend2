...
describe("Cart test", () => {
  ...
  describe("Checkout", () => {
    ...
    it("should throw 400 error if `wallet balance is insufficien`t", async () => {
    ...
    it("should update user balance and empty the cart on success", async () => {
      ...
      const newBalance = userOne.walletMoney - cartWithProductsUserOne.cartItems.reduce((sum, currentItem) => sum + currentItem.quantity*currentItem.product.cost, 0); 
      expect(userOneFinal.walletMoney).toEqual(newBalance); 
    });
  });
});
