...
const generateToken = (userId, expires, type, secret = config.jwt.secret) => {
  return jwt.sign({sub: userId, expires, type}, secret); 
};
...
const generateAuthTokens = async (user) => {
  const expiresDateObj = new Date(Date.now() + config.jwt.accessExpirationMinutes*60*1000); 
  const expiresInSeconds = expiresDateObj.getTime()/1000; 

  const token = generateToken(user._id, expiresInSeconds, tokenTypes.ACCESS, config.jwt.secret); 
  return {
    "access": {
      expires: expiresDateObj,
      token
    }}; 
};
